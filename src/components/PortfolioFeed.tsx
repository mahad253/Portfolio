                  ...
                  <div className="relative flex-[0_0_100%]" key={index}>
                    <div className="w-full">
                      {image.endsWith('.webm') ? (
                          <video
                              src={image}
                              className="max-h-[550px] w-full mx-auto object-contain bg-white rounded-md border"
                              autoPlay
                              loop
                              muted
                              playsInline
        ...
        <div className="flex gap-2 overflow-auto pb-2">
          {images.map((image, index) => (
              <button
                  key={index}
                  className={cn(
                      "relative flex-0 min-w-[130px] cursor-pointer overflow-hidden rounded-md border-2 transition-all",
                      selectedIndex === index
                          ? "border-primary"
              ...
              >
                <div className="aspect-video w-[130px]">
                  {image.endsWith('.webm') ? (
                      <video
                          src={image}
                          className="h-full w-full object-contain bg-white border rounded-md hover:scale-105 transition-transform duration-300"
                          autoPlay
                          loop
                          muted
                          playsInline
          ...
          <div className="space-y-4">
            ...
            >
              <div className="rounded-lg bg-muted/50 p-4 space-y-4">
                <div>
                  <h4 className="font-semibold mb-2">Défi</h4>
